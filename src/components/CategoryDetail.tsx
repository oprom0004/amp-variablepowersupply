...
export default function CategoryDetail() {
  ...
  useEffect(() => {
    if (category) {
      document.title = `${category.name} | Professional Variable Power Supply`;
    }
  }, [category]);
  ...
  if (!category) {
    return (
      <div className="min-h-screen bg-[#F9F9F8] text-[#1A1A1A] flex flex-col items-center justify-center p-8">
        ...
        <Link to="/" className="bg-[#1A1A1A] text-white px-8 py-4 font-sans text-[10px] font-black uppercase tracking-[0.2em]">Return to Catalog</Link>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-10">
        <div className="flex justify-between items-center mb-8">
           <nav className="flex items-center space-x-2 font-sans text-[10px] font-black uppercase tracking-[0.2em]">
             <Link to="/" className="text-gray-400 hover:text-[#C53030] transition-colors">Catalog_Index</Link>
          ...
          <div className="p-10 md:p-14 border-r-[0.5px] border-[#1A1A1A] flex flex-col justify-center space-y-12">
            <div>
              ...
              <div className="space-y-8">
                ...
                >
                  <span>VIEW ALL {category.name.replace('Variable Power Supply', '').trim() || 'SERIES'} MODELS</span>
            ...
            <div className="grid grid-cols-2 gap-8 border-t border-gray-100 pt-10">
               <div className="space-y-1">
                 ...
                 <div className="font-sans text-4xl font-black tracking-tighter italic">
                   {category.name.match(/\d+/)?.[0] || '800'}<span className="text-sm ml-1 opacity-20 NOT-ITALIC">V_DC</span>
                 </div>
               </div>
               <div className="space-y-1">
                 <div className="font-sans text-[9px] font-black uppercase tracking-[0.3em] text-gray-300 italic">Regulation_Acc</div>
                 <div className="font-sans text-4xl font-black tracking-tighter italic">
                   ±0.01<span className="text-sm ml-1 opacity-20 NOT-ITALIC">%/FS</span>
                 </div>
               </div>
            </div>
          </div>
          ...
          <div className="bg-[#F9F9F8] relative overflow-hidden flex flex-col min-h-[500px]">
            ...
            <div className="flex-1 relative flex items-center justify-center p-12">
               ...
               <motion.div 
                 ...
                 className="relative z-10 w-full aspect-video grayscale hover:grayscale-0 transition-all duration-1000 shadow-[20px_20px_0px_0px_rgba(26,26,26,0.03)]"
               >
                 <img 
                   src={`https://picsum.photos/seed/${category.slug}/1200/675`}
                   alt={`${category.name}`}
                   referrerPolicy="no-referrer"
                   className="w-full h-full object-cover border border-[#1A1A1A]"
                 />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
              ...
              <div className="lg:col-span-7 bg-white border border-[#1A1A1A] p-12 md:p-16 relative">
                 ...
                 <div className="mt-12 pt-8 border-t border-gray-100 flex items-center justify-between">
                    ...
                    <Link to="/pricing" className="font-sans text-[10px] font-black uppercase tracking-[0.2em] text-[#C53030] hover:underline">Price_Review_→</Link>
                 </div>
              </div>
            </div>
          </div>
        ...
        <div className="mt-32 pt-24 border-t border-[#1A1A1A]">
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16">
              ...
              <Link to="/" className="font-sans text-[10px] font-black uppercase tracking-[0.3em] bg-[#1A1A1A] text-white px-10 py-5 hover:bg-[#C53030] transition-colors">View_Full_Index_→</Link>
           </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-[#1A1A1A]/10 border border-[#1A1A1A]/10">
              {CATEGORIES.filter(c => c.slug !== category.slug)
                ...
                .slice(0, 4)
                .map((rel, i) => (
                   <Link key={rel.slug} to={`/${rel.slug}`} className="group bg-white p-10 space-y-8 hover:bg-[#F9F9F8] transition-colors flex flex-col">
           ...
           <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
              <div className="lg:col-span-4 space-y-8">
                 ...
                 <h3 className="font-sans text-3xl font-black uppercase tracking-tighter leading-none italic">
                    Technical Insights: <br /> {category.name.replace('Variable Power Supply', '').trim() || 'Professional Series'}
                 </h3>
              ...
              <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
                 {(category.faqs || [
                   {
                     q: `Why choose this ${category.name.replace('Variable Power Supply', '').trim() || 'Variable'} series for precision work?`,
